...
class TSClusteringLayer(Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        if self.dist_metric == 'eucl':
            ...
            # Step-by-step translation to TensorFlow using tf.norm
            expanded_inputs = tf.expand_dims(inputs, axis=1)  # Shape (batch_size, 1, input_dim)

            # Compute the Euclidean distance (L2 norm) between inputs and clusters
            distance = tf.reduce_sum(tf.norm(expanded_inputs - self.clusters, ord='euclidean', axis=-1), axis=-1)
