...
from keras.layers import Input, Conv1D, LeakyReLU, MaxPool1D, CuDNNLSTM, Bidirectional, TimeDistributed, Dense, Reshape, LSTM
...
def temporal_autoencoder(input_dim, timesteps, n_filters=50, kernel_size=10, strides=1, pool_size=10, n_units=[50, 1]):
    ...
    decoded = UpSampling2D((pool_size, 1), name='upsampling', interpolation = 'bilinear')(decoded)  #decoded = UpSampling1D(pool_size, name='upsampling')(decoded)
...
def temporal_autoencoder_v2(input_dim, timesteps, n_filters=50, kernel_size=10, strides=1, pool_size=10, n_units=[50, 1]):
    ...
    decoded = UpSampling2D((pool_size, 1), name='upsampling', interpolation = 'bilinear')(decoded)
    decoded = Conv2DTranspose(input_dim, (kernel_size, 1), padding='same', name='conv2dtranspose')(decoded)
